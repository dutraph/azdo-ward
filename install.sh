#!/usr/bin/env bash
# install.sh — build and install azdo-ward into PREFIX/bin.
#
# Usage:
...
REPO_URL="${REPO_URL:-https://github.com/dutraph/azdo-ward.git}"
REF="${REF:-main}"
...
src_dir=""
if [ -f "./go.mod" ] && grep -q "^module github.com/dutraph/azdo-ward" ./go.mod 2>/dev/null; then
    ...
    git clone --depth 1 --branch "$REF" "$REPO_URL" "$tmp/azdo-ward"
    src_dir="$tmp/azdo-ward"
fi
...
log "compiling azdo-ward $version"
mkdir -p "$src_dir/bin"
( cd "$src_dir" && go build \
    -trimpath \
    -ldflags "-s -w -X github.com/dutraph/azdo-ward/internal/version.Version=${version#v}" \
    -o ./bin/azdo-ward ./cmd/azdo-ward )

log "installing to $INSTALL_DIR/azdo-ward"
$SUDO install -m 0755 "$src_dir/bin/azdo-ward" "$INSTALL_DIR/azdo-ward"
